...
class DbObject
{
    ...
    public function  softDelete() {
        global $database;
        $sql = "UPDATE " . static::$db_table . " SET timestamp = CURRENT_TIMESTAMP() WHERE id= " . $database->escape_string($this->id);
        $database->query($sql);
        return (mysqli_affected_rows($database->connection) == 1) ? true : false;
    }
    public function un_delete() {
        global $database;
        $sql = "UPDATE " . static::$db_table . " SET timestamp = NULL WHERE id= " . $database->escape_string($this->id);
        $database->query($sql);
        return (mysqli_affected_rows($database->connection) == 1) ? true : false;
    }
